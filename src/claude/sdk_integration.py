...
class ClaudeSDKManager:
    ...
    def _build_system_prompt(self, working_directory: Path) -> str:
        ...
        return (
            ...
            "- Slack: `python3 scripts/slack_api.py post CHANNEL 'msg'` "
            "(add `--as george` for George's identity)\n"
            "- Asana: `python3 scripts/asana_api.py`\n"
            "- HubSpot: `python3 scripts/hubspot_api.py search-deals` / `recent-activity`\n"
            "- Clay: `python3 scripts/clay_api.py search 'Name'`\n"
            "- Exa: `python3 scripts/exa_api.py search 'query'`\n"
            "## Obsidian Vault\n"
            "- Inbox tasks: `python3 scripts/obsidian_write.py inbox '- [ ] 🟡 Task description 📅 YYYY-MM-DD' --section '🟡 This Week'`\n"
            "  Sections: '🔴 Needs Action Today', '🟡 This Week' (default), '👥 Delegated — Awaiting Response', '🟢 In Progress / Monitor'\n"
            "- Quick notes: `python3 scripts/obsidian_write.py scratchpad 'note'`\n"
            "- NEVER write directly to INBOX.md — always use obsidian_write.py inbox\n"
            "- Granola meetings: `python3 scripts/search_meetings.py 'query' --since 7` "
            "(search by participant/keyword)\n"
            "  `python3 scripts/search_meetings.py --latest` (most recent meeting)\n"
            "  `python3 scripts/search_meetings.py --id DOC_ID --transcript` "
            "(specific meeting with transcript)\n"
            "- Task dispatch: `python3 scripts/task_dispatch.py add 'description' "
            "--project scout` (dispatch task to VPS Claude)\n"
            "  `python3 scripts/task_dispatch.py status` (check task status)\n\n"

            "## Key Files\n"
            "- Employee directory: memory/WB-EMPLOYEE-DIRECTORY.md\n"
            "- Scratchpad: memory/SCRATCHPAD.md\n"
            "- Exec todo: Asana (via scripts/asana_api.py)\n\n"

            "## Browser Automation (Firecrawl)\n"
            "You can browse the web using Firecrawl browser sandbox:\n"
            "Create: SID=$(python3 scripts/firecrawl_api.py browser create | "
            "python3 -c \"import sys,json; print(json.load(sys.stdin)['id'])\")\n"
            "Navigate: python3 scripts/firecrawl_api.py browser exec $SID \"code\" --lang python\n"
            "Screenshot: python3 scripts/firecrawl_api.py browser screenshot $SID /tmp/shot.png\n"
            "Close: python3 scripts/firecrawl_api.py browser close $SID\n"
